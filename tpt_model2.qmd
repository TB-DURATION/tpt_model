...
Looking at the way to see everything regarding prophylaxis:
...
# 1. treatment uptake
  ds <- seq2(0, dur_max)
  plot2(ds, uptake(ds, Xv, Yv, Zv, hv), ylim = 0:1,
        xlab = "duration of treatment (days)", ylab = "uptake probability")

# 2. conditional dropping probability
  plot_probabilities(pd, Xf, Yf, hf, Xmp, Ym, hm,
                     lwd = 2, ylim = 0:1, add = FALSE, lgnd = TRUE)

# 3. actual duration of treatment
  effective_duration(pd, Xf, Yf, hf, Xmp, Ym, hm, by, le) |> 
    with({
      plot3(x, y, ylab = "probability density")
      polygon2(x, y)
    })
...
# 4. pharmaco-dynamics treatment efficacy
  ts <- seq2(0, dur_max)
  plot3(ts, hill(ts, Xe, Ye, he), ylim = 0:1, ylab = "treatment efficacy")
...
# 5. 
  ...
  out <- mclapply2(pd_vals,
                   function(x) pi_value(tau, x, Xv, Yv, hv, Zv, Xf, Yf, hf, Xmp, Ym,
                                        hm, Xe, Ye, he, by, le, n))
  quantplot(pd_vals, out,  probs = seq(.025, .975, le = 75), col = adjustcolor(4, .1))
}

opar <- par(mfrow = c(2, 3), cex = 1, plt = c(.25, .95, .25, .9))
dashboard()
par(opar)
```
...
```{r}
dashboard <- function(
    proposed_durations = seq(1, 40, le = 7),
    tau = .99,
    ...
    le = 512, # nb of pts on the lines of fig 2, along the x axis of fig 4
  ...
  proposed_durations <- sort(proposed_durations)
  max_dur <- max(proposed_durations)
  ds <- seq2(0, max_dur)
...
# 2. actual durations of treatment
  eds <- map(proposed_durations,
             ~ effective_duration(.x, Xf, Yf, hf, Xmp, Ym, hm, le = le))
  plot(NA, xlim = c(0, max(proposed_durations)),
           ylim = c(0, max(unlist(map(eds, ~ .x$y)))),
...
# 3. pharmaco-dynamics treatment efficacy
  plot3(ds, hill(ds, Xe, Ye, he), ylim = 0:1, ylab = "treatment efficacy")
...
# 4. intervention efficacy
  ...
  pd_vals <- seq(1, max_dur, le = le) #
  out <- mclapply2(pd_vals,
                   function(x) pi_value(tau, x, Xv, Yv, hv, Zv, Xf, Yf, hf, Xmp, Ym,
                                        hm, Xe, Ye, he, le = npop, n = nspl))
  quantplot(pd_vals, out,  probs = probs, col = adjustcolor(4, alpha))
}
```
...
seq(0, 1, .1) |> 
  map(pi_effect) |> 
  walk(~ with(.x, lines2(time, d, col = 4)))
```
