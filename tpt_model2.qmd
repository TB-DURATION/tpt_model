...
```{r}
ds <- seq2(0, 30)
plot2(ds, uptake2(ds, 15, 1, 1, 4), ylim = 0:1,
      xlab = "duration of treatment (days)", ylab = "uptake probability")
lines2(ds, uptake2(ds, 10, .7, .9, 1.75), col = 3)
```
...
add_legend_sterilization <- function(...) {
  legend2("topright", legend = paste0(rev(100 * tau_vals), "%"),
          ...
          title = expression(paste("perentage ", tau, " of recent infections identified")))
}
