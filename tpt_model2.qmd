...
legend4 <- function(x, y, legend, cex, ...) {
  tmp <- legend(x, y, legend = rep(" ", length(legend)), bty = "n",
                text.width = max(strwidth(legend)), xjust = 1, yjust = 1, cex = cex, ...)
  text(tmp$rect$left + tmp$rect$w, tmp$text$y, legend, pos = 2, cex = cex)
}
...
# 1. duration of "recent infections"
  ...
  text(2.95, .5, "long-lasting infections", col = greys[3], 1, font = 2)
  text(2.95, .15, "disease", col = skin_tones[3], 1, font = 2)
...
# 2. duration of disease
  ...
  cols <- colorRampPalette(yellows)(10)
  polygon3(xs, ys, rep(0, length(xs)), col = cols[1])
  
  qtls <- qexp(seq(.1, .9, .1), alpha)
  for (i in 1:length(qtls)) {
    sel <- qtls[i] <= xs
    polygon3(xs[sel], ys[sel], rep(0, length(xs[sel])), col = cols[i + 1])
  }
  ...
  text(.9 * xmax, .93 * ymax,
       bquote(paste("R"[0], " = ", .(round(N * p_val[["beta"]] / alpha, 2)))), 1)
  text(.9 * xmax, .78 * ymax,
       paste("Pr[d|oi] =", round(delta2 / (delta2 + gamma2), 2)), 1)
  text(.9 * xmax, .63 * ymax, paste("median =", round(log(2) / alpha, 2)), 1)
  text(.95 * xmax, .605 * ymax, "yr")
  text(.9 * xmax, .48 * ymax, paste("mean =", round(1 / alpha, 2)), 1)
  text(.95 * xmax, .455 * ymax, "yr")
...
# 3. decrease in disease incidence    
  ...
  legend4("topright", lty = 2:4, lwd = 2, cex = .8,
         legend = paste0(formatC(round(100 * c(.02, .1, .2)),
                                 digits = 0, width = 2, format = "f"), "%/yr"))

# 4.
  inc_at_time(I0, D0, di, x, delta1, gamma1, gamma2, N, horizons, times1)
  cols <- colorRampPalette(greens)(7)
  cols <- c(cols[1:2], "white", cols[7])
  legend4("topright", legend = c("2025", "2030", "...", "2055"),
          lty = 1, lwd = 2, cex = .8, col = cols)
...
# 5. decrease rate
  ...
  legend4("topright", title = "1 - p", legend = c("0.00", "0.10", "...", "1.00"),
          lty = 1, lwd = 2, cex = .8, col = cols)
...
# 6. time to target
  ...
  legend4("topright", title = "incidence", legend = c("0.00", "0.10", "...", "1.00"),
          lty = 1, lwd = 2, cex = .8, col = cols)
