...
```{r}
siid_equ2(N = 1e5, p = 1, beta = .00001, gammas = 1 / 70, gammaf = 0, deltas = 1 / 50, deltaf = 1 / 5, alpha = 1 / 70)
```
...
If $\gamma_s = \gamma_f = 0$, then we have:

$$
\begin{align} 
  D^*    &= \frac{(\beta N - \alpha)\delta_s\delta_f}
                 {(\alpha + \delta_s)\delta_f - (\delta_f - \delta_s)\alpha p\beta}   \\
...
\begin{align}
  d^*      &= d_s^* + d_f^*                                                           \\
  B        &= {d_f^*}^2 - (\delta_fI^* - d_s^*)d_f^*                                  \\
  \alpha   &= \frac{d^*}{D^*}                                                         \\
  \delta_s &= \frac{\delta_fd_s^*}{\delta_f I^* - d_f^*}                              \\
  \beta    &= \frac{(D^*S^* - d^*)d^*d_f^* +
                    [(d_s^*N - d^*I^*)d^* + 
                     (I^*S^*-d_s^*)d^*D^* + 
                     (d_s^*N - [d_f^* + 2d_s^*]I^*){D^*}^2 - 
                     d_s^*{D^*}^3] \delta_f}{\delta_f d_s^*S^*D^*N}                   \\
  p        &= \frac{d_f^*d_s^*\delta_f N}
                   {(B + (N - I^*)\delta_f d_s^*)d + 
                    ((I^*N - d_s^* - {I^*}^2)\delta_f - (N - I^*)d_f^*)d^*D^* +
                    (B - (N - 2I^*)\delta_f d_s^*){D^*}^2 -
                    \delta_f d_s^*{D^*}^3                                             
                   }
\end{align}
$$
...
siid_param_val <- function(I, D, ds, df, N, deltaf) {
  ...
  c(alpha  = d / D,
    ...
    p      = (df *ds * deltaf * N) / 
             (
               (B + (N - I) * deltaf * ds) * d +
               ((I * N - ds - I^2) * deltaf - (N - I) * df) * d * D +
               (B - (N - 2 * I) * deltaf * ds) * D^2 -
               deltaf * ds * D^3   
             )
  )
}
...
di <- 322
df <- round(x * 322)
siid_param_val(I = 1e4, D = 176, ds = di - df, df = df, N = 1e5, deltaf = 1 / 2)
siid_param_val(I = 3e4, D = 176, ds = di - df, df = df, N = 1e5, deltaf = 1 / 2)
```
...
```{r}
ss <- function(p, beta, gammas, gammaf, deltas, deltaf, alpha, D, I, df, ds) {
  equ <- list(beta = beta, alpha = alpha,
              gammas = gammas, gammaf = gammaf,
              deltas = deltas, deltaf = deltaf) |> 
    map(exp) |> 
    c(p = logistic(p), N = 1e5) %>% 
    do.call(siid_equ2, .)
  (D - equ[["D"]])^2 + (I - equ[["I"]])^2 + (df - equ[["df"]])^2 + (ds - equ[["ds"]])^2
}
```

```{r}
list(beta = 1e-4, alpha = 1 / 70, gammas = 1 / 70, gammaf = 1 / 70,
     deltas = 1 / 10, deltaf = 1 / 2) |> 
  map(log) |> 
  c(list(p = logit(.9), D = 300, I = 10000, df = 270, ds = 30)) %>%
  do.call(ss, .)
```

```{r}
ss(-54.3664224, -15.5551976, -11.2938616, 40.0465960, -2.3025851, -.6931472, -4.2484952,
   D = 300, I = 10000, df = 270, ds = 30)
```

```{r}
mle2(ss,
     as.list(c(p = logit(.9), log(c(beta = 1e-4, gammas = 1 / 70, gammaf = 1 / 70)))),
     fixed = as.list(log(c(alpha = 1 / 70, deltas = 1 / 10, deltaf = 1 / 2))),
     data = list(D = 300, I = 10000, df = 270, ds = 30))
```
