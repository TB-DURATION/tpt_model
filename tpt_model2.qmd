...
```{r}
find_rate <- function(x0 = 176, p = .1, b1 = .5 * p, b2 = 1.5 * p) {
  optimise(function(x) abs(x0 * exp(- x) - (1 - p) * x0), c(b1, b2))$min
}
```
...
pi2dis_inc <- function(
  ...
  if (show_rates) {
    xs <- seq(mint, maxt, by)
    walk2(rates, 2:4,
          ~ lines2(xs, di * exp(- find_rate(di, .x) * (xs - mint)), lty = .y))
  }
...
\begin{align} 
  ...
  \frac{dI_2}{dt} &= \sigma I_2 - (\delta_2 + \gamma_2) I_2 \\
