...
\begin{align} 
  S^* &= N - I^* - D^*         \\
  I^* &= \frac{\alpha + \gamma + \rho + \mu}{\delta} D^*  \\
  D^* &= \frac{\beta \delta N - (\delta + \rho + \mu) \mu - (\rho + \mu) \sigma - (\delta + \sigma + \mu) (\alpha + \gamma)}{(\alpha + \delta + \gamma + \rho + \mu) \beta}
\end{align}
$$
...
```{r}
assign_parameters_values <- function(f) {
  function(...) f(..., beta  = 1.2e-5,
                       sigma = 0, #.1,
                       delta = - log(1 - .9) / 2,
                       gamma = 1 / 1.5,
                       rho   = 0, #.15,
...
sid_dyn_sim <- assign_parameters_values(sid_dyn)(S0    = N - pvlc,
                                                 I0    = 0,
                                                 D0    = pvlc,
                                                 times = seq2(0, 100))

sid_equ_val <- assign_parameters_values(sid_equ)(N = N)
